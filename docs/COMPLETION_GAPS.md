...
- Status: blocked (2026-09-26). Priority: P2. Category: usability. Blocked on a requirement automation cannot satisfy: the 2025.33230 leg needs a licensed Derivative build that is not installed on the only TouchDesigner host and that this harness cannot obtain (no licensed download channel, no host install capability). Every 2025.32820 criterion is met and recorded.
...
- Acceptance criteria: met on 2025.32820 — installed artifact hash (runtime `9c4f1f5a…`, saved project `908296be…`), interaction steps, meaningful output, recovery result, and cleanup result all retained in section 3. Not met for the 2025.33230 leg, so the entry is blocked with the reason above.
