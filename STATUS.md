...
*Incrementally synced 2026-09-19 to reference `2df19feb` (`f1d2b46a2773..2df19feb6ce143636809b1e581699a7110b1f94d`) — audited upstream commit `2df19feb6ce1` (adds borrowed `VideoFrame` support to `updateTextureFromSource` in WebGL2 and WebGPU browser backends). TouchDesigner executes native TOP pipelines (Movie File In TOP, Video Device In TOP) and does not consume browser DOM or WebCodecs media APIs. Verified zero effect definitions, shaders, or compiler operations changed upstream (213/213 definitions byte-identical via `tools/convert-definitions.mjs`). Verified compiler parity gates: check_lex (329/329 PASS), check_parse (329/329 PASS), check_validate (329/329 PASS), check_graph (328 PASS / 0 DIFF / 0 STAGE / 1 SKIP), and unit test suite (64/64 PASS).*

*Incrementally synced 2026-09-20 to reference `beabda38` (`2df19feb6ce1..beabda385253a3461d2ee5ee2f1b032cbe9a2832`) — ported upstream `beabda38` MIDI validator fix in `td/noisemaker/compiler/lang/validator.py` to unconditionally enforce static integer channels 1..16 across all channel-based MIDI modes. Audited upstream `6e0166ce` (dynamic surface/texture format checks and recreation in the JS runtime pipeline); TouchDesigner uses a static TOP network compiled once and re-cooked per frame, where TOP formats are declared by pass specifications. Added unit tests in `parity/test_midi_expression.py` verifying legacy MIDI note modes reject non-integer / out-of-range channels (0, 17, 1.5, true, "1", osc()) and accept 1 and 16. Verified compiler parity gates: check_lex (329/329 PASS), check_parse (329/329 PASS), check_validate (329/329 PASS), check_graph (328 PASS / 0 DIFF / 0 STAGE / 1 SKIP), and unit test suite (65/65 PASS).*

*Incrementally synced 2026-09-21 to reference `f61ac073` (`beabda385253..f61ac0732088`) — removed expired filter effects `filter/bc`, `filter/colorspace`, and `filter/hs` (upstream commit `2f855c9c`) following migration to `filter/adjust`. Removed definitions `td/noisemaker/effects/filter/{bc,colorspace,hs}.json`, shaders `td/noisemaker/shaders/effects/filter/{bc,colorspace,hs}/`, test programs `parity/programs/{bc,colorspace,hs}.dsl`, and ledger entries in `parity/ledger.tsv`. Regenerated definitions via `tools/convert-definitions.mjs` (210/210 effects byte-identical) and shaders via `tools/convert-shaders.mjs` (301/301 programs). Added unit test in `parity/test_validator_contract.py` asserting `filter/adjust` compiles and expired effects `bc`, `colorspace`, and `hs` produce diagnostic `S001`. Verified compiler parity gates: check_lex (326/326 PASS), check_parse (326/326 PASS), check_validate (326/326 PASS), check_graph (325 PASS / 0 DIFF / 0 STAGE / 1 SKIP), and unit test suite (`./parity/.venv/bin/python3 -m unittest discover -s parity -p "test_*.py"`, 66/66 PASS).*

*Incrementally synced 2026-09-21 to reference `50b8f909` (`f61ac0732088..50b8f909ff59`) — ported upstream GAP-001 DSL output surface range enforcement into `td/noisemaker/compiler/lang/lexer.py`: output references must be in range `o0..o7` unless preceded by a DOT token (member segment access, e.g. `foo.o8`); out-of-range references throw `DslSyntaxError`. Updated `reference/01-dsl-frontend.md` section 1.4. Added unit tests in `parity/test_validator_contract.py` verifying boundary behavior (`o0`, `o7`), rejection of out-of-range surfaces (`o8`, `o12`, `o99`), member segment exceptions (`foo.o8`, `foo.o99`), and multi-digit retention for other surface reference families (`s99`, `vol99`, etc.). Verified compiler parity gates: check_lex (326/326 PASS), check_parse (326/326 PASS), check_validate (326/326 PASS), check_graph (325 PASS / 0 DIFF / 0 STAGE / 1 SKIP), and unit test suite (`./parity/.venv/bin/python3 -m unittest discover -s parity -p "test_*.py"`, 67/67 PASS).*

*Incrementally synced 2026-09-22 to reference `e5bd2013` (`50b8f909ff59..e5bd2013087e`) — ported upstream DSL diagnostic column location preservation into `td/noisemaker/compiler/lang/validator.py`: diagnostics preserve exact `{ line, column }` source locations and prefer explicit `column` over `col` when present. Audited upstream `68d37721` (excluding builtins from interactive AST mutation introspection `listSteps`, `replaceEffect`, `getCompatibleReplacements`), inapplicable to TouchDesigner's compile-only pipeline. Verified definitions conversion via `tools/convert-definitions.mjs` (210/210 effects byte-identical). Added unit tests in `parity/test_validator_contract.py` asserting compilation diagnostics preserve exact line/column locations across AST nodes and caller-supplied column overrides. Verified compiler parity gates: check_lex (326/326 PASS), check_parse (326/326 PASS), check_validate (326/326 PASS), check_graph (325 PASS / 0 DIFF / 0 STAGE / 1 SKIP), and unit test suite (`./parity/.venv/bin/python3 -m unittest discover -s parity -p "test_*.py"`, 68/68 PASS).*

*Incrementally synced 2026-09-22 to reference `643b2be1` (`e5bd2013087e..643b2be1e28b`) — ported upstream structured DSL lexer diagnostics into `td/noisemaker/compiler/lang/diagnostics.py`, `td/noisemaker/compiler/lang/dsl_syntax_error.py`, and `td/noisemaker/compiler/lang/lexer.py`: `DslSyntaxError` carries structured `diagnostic` dictionary with `code` (`L001`..`L004`), `stage: 'lexer'`, `severity: 'error'`, `message`, `location: { line, column }`, and `span: { start, end }`. Added `stage()` helper and registered `L001`..`L004`, `P001`..`P002`, and `R001` in `diagnostics.py`. Added unit tests in `parity/test_validator_contract.py` verifying structured diagnostics and span/location coordinates for unexpected characters (`L001`), unterminated string literals and triple-quoted strings (`L002`), unclosed comments (`L003`), and out-of-range output surfaces (`L004`). Verified definitions conversion via `tools/convert-definitions.mjs` (210/210 effects byte-identical). Verified compiler parity gates: check_lex (326/326 PASS), check_parse (326/326 PASS), check_validate (326/326 PASS), check_graph (325 PASS / 0 DIFF / 0 STAGE / 1 SKIP), and unit test suite (`./parity/.venv/bin/python3 -m unittest discover -s parity -p "test_*.py"`, 69/69 PASS).*

*Incrementally synced 2026-09-22 to reference `44bc4ed4` (`643b2be1e28b..44bc4ed4ac72`) — regenerated effect definitions via `tools/convert-definitions.mjs` (210/210 effects) and shaders via `tools/convert-shaders.mjs` (301/301 programs); `render/renderLandscape3d` now includes `filtering` parameter (`define: "FILTERING"`, choices: `isosurface: 0, voxel: 1`, default: 1) and GLSL landscape isosurface raymarching path. Updated `td/noisemaker/compiler/lang/parser.py` and `dsl_syntax_error.py` to emit structured parser expectation diagnostics `P001` (unexpected token) and `P002` (expected closing parenthesis) with exact token locations or explicit `location: None, span: None` for unlocated caller tokens and string representations (`"undefined"`, `"NaN"`). Updated `td/noisemaker/compiler/lang/lexer.py` column and span calculations to strictly count UTF-16 code units. Added unit tests in `parity/test_validator_contract.py` covering parser expectation diagnostics, unavailable caller coordinates, and `renderLandscape3d` filtering defines. Verified compiler parity gates: check_lex (326/326 PASS), check_parse (326/326 PASS), check_validate (326/326 PASS), check_graph (325 PASS / 0 DIFF / 0 STAGE / 1 SKIP), and unit test suite (`./parity/.venv/bin/python3 -m unittest discover -s parity -p "test_*.py"`, 72/72 PASS).*

*Incrementally synced 2026-09-23 to reference `3a32b198` (`44bc4ed4ac72..3a32b198efea`) — ported upstream structured DSL parser diagnostics `P003` (automation arguments) and `P004` (search directives) into `td/noisemaker/compiler/lang/diagnostics.py` and `td/noisemaker/compiler/lang/parser.py`: `DslSyntaxError` carries structured `diagnostic` dictionary with `code`, `stage: 'parser'`, `severity: 'error'`, `message`, `location: { line, column }`, and `span: None`. Registered `P003` and `P004` in `diagnostics.py`. All `osc`, `midi`, and `audio` argument/keyword validation sites now throw `P003` diagnostics; missing, duplicate, misplaced, nested, and invalid `search` directive sites now throw `P004` diagnostics with exact token coordinates or explicit null locations for degraded/synthetic tokens. Added unit tests in `parity/test_validator_contract.py` verifying `P003` (22 automation cases), `P004` (10 search directive cases), UTF-16 surrogate pairs, CRLF/tabs, and unlocated caller coordinates. Regenerated effect definitions via `tools/convert-definitions.mjs` (210/210 effects byte-identical). Verified compiler parity gates: check_lex (326/326 PASS), check_parse (326/326 PASS), check_validate (326/326 PASS), check_graph (325 PASS / 0 DIFF / 0 STAGE / 1 SKIP), and unit test suite (`./parity/.venv/bin/python3 -m unittest discover -s parity -p "test_*.py"`, 75/75 PASS).*

*Incrementally synced 2026-09-23 to reference `5b81e04f` (`3a32b198efea..5b81e04f8a4b`) — ported upstream shader performance optimizations into `classicNoisedeck/noise` (`fde2ea40`) and `classicNoisedeck/glitch` (`9b88e567`) bypassing refraction noise lookups, glitch refraction, scanlines, and snow when corresponding amounts are zero. Regenerated effect definitions via `tools/convert-definitions.mjs` (210/210 effects byte-identical). Ported upstream output sink deferral semantics (`5b81e04f`): added `should_defer_render()` (with `shouldDeferRender` alias) to `SinkManager` (`td/noisemaker/runtime/sink.py`), `Pipeline` (`td/noisemaker/runtime/pipeline.py`), and `NMRenderer` (`td/noisemaker/runtime/nm_renderer.py`), supporting both `defer_render` and `deferRender` sink protocols with full failure isolation and tombstone compaction. Added unit tests in `parity/test_output_runtime.py` covering sink deferral, removal, closed manager, error isolation on throwing sinks, and delegation through `Pipeline` and `NMRenderer`. Verified compiler parity gates: check_lex (326/326 PASS), check_parse (326/326 PASS), check_validate (326/326 PASS), check_graph (325 PASS / 0 DIFF / 0 STAGE / 1 SKIP), and unit test suite (`./parity/.venv/bin/python3 -m unittest discover -s parity -p "test_*.py"`, 76/76 PASS).*

*Incrementally synced 2026-09-24 to reference `c9ee8a04` (`5b81e04f8a4b..c9ee8a049b2b`) — ported upstream structured DSL parser diagnostics `P005` (invalid output operation) into `td/noisemaker/compiler/lang/diagnostics.py` and `td/noisemaker/compiler/lang/parser.py`: registered `P005` in `_TABLE`; attached structured `diagnostic` dictionary with `code: 'P005'`, `stage: 'parser'`, `severity: 'error'`, `message`, `location: { line, column }`, and `span: None` across five explicit output validation sites: render target, write and write3d in expression context, write surface, write3d texture, and write3d geometry. Verified definitions conversion via `tools/convert-definitions.mjs` (210/210 effects byte-identical). Added unit tests in `parity/test_validator_contract.py` verifying `P005` diagnostics across 13 failure cases, multiline/CRLF/tab/UTF-16 locations, unavailable caller coordinates, shared expectation precedence (`P001`/`P002`), and valid output AST forms. Verified compiler parity gates: check_lex (326/326 PASS), check_parse (326/326 PASS), check_validate (326/326 PASS), check_graph (325 PASS / 0 DIFF / 0 STAGE / 1 SKIP), and unit test suite (`./parity/.venv/bin/python3 -m unittest discover -s parity -p "test_*.py"`, 80/80 PASS).*

*Incrementally synced 2026-09-24 to reference `13fa8b54` (`c9ee8a049b2b..13fa8b540025`) — ported upstream structured DSL parser diagnostics `P006` (invalid subchain) into `td/noisemaker/compiler/lang/diagnostics.py` and `td/noisemaker/compiler/lang/parser.py`: registered `P006` in `_TABLE`; attached structured `diagnostic` dictionary with `code: 'P006'`, `stage: 'parser'`, `severity: 'error'`, `message`, `location: { line, column }`, and `span: None` across subchain validation sites (non-number iterations, non-string argument name/id, missing chain dot, empty body). Added unit tests in `parity/test_validator_contract.py` verifying `P006` diagnostics across 9 failure cases, multiline/CRLF/tab/UTF-16 locations, unavailable caller coordinates, shared expectation precedence (`P001`/`P002`), and valid subchains AST forms. Verified compiler parity gates: check_parse (326/326 PASS), and unit test suite (`./parity/.venv/bin/python3 -m unittest discover -s parity -p "test_*.py"`, 84/84 PASS).*

*Incrementally synced 2026-09-24 to reference `fca611fd` (`13fa8b540025..fca611fd8f91`) — ported upstream structured DSL parser diagnostics `P007` (invalid call expression), `P005` (duplicate render), remaining `P001` expectation sites, and number coercion location preservation into `td/noisemaker/compiler/lang/diagnostics.py` and `td/noisemaker/compiler/lang/parser.py`: registered `P007` in `_TABLE`; attached structured `diagnostic` dictionary with `code: 'P007'`, `stage: 'parser'`, `severity: 'error'`, `message`, `location: { line, column }`, and `span: None` across call validation sites (`from()` named arguments, argument count, non-identifier namespace, non-call second argument, inline namespace syntax, and mixed positional/keyword arguments). Updated duplicate `render()` to emit `P005` and remaining expectation sites (assignment RHS, kwarg RHS, closing bracket, dot member identifier, unexpected token) to emit `P001`. Updated `_to_number` to preserve location metadata when coercing non-number nodes with attached coordinates (e.g. `ArrayLiteral`) and explicitly emit null location for unlocated operands. Verified definitions conversion via `tools/convert-definitions.mjs` (210/210 effects byte-identical). Added unit tests in `parity/test_validator_contract.py` verifying `P007` call forms, unlocated caller coordinates, remaining `P001` expectations, number coercion location preservation, and valid call forms with from-override namespaces and mixed midi automation. Verified compiler parity gates: check_lex (326/326 PASS), check_parse (326/326 PASS), check_validate (326/326 PASS), check_graph (325 PASS / 0 DIFF / 0 STAGE / 1 SKIP), and unit test suite (`./parity/.venv/bin/python3 -m unittest discover -s parity -p "test_*.py"`, 91/91 PASS).*
